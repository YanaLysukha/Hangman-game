...
export default class Router {
    ...
    private constructor() {
        this.currentRoutePath = window.location.pathname.slice(
            1,
        ) as PageRouteKey;
        this.#list = ROUTES;
    ...
    addPopStateEventListener = () => {
        window.addEventListener("popstate", (event) => {
            if (event.state) {
                this.route(
                    window.location.pathname.slice(1) as PageRouteKey,
                    true,
                );
            }
        });
    ...
    route = (routePath = this.currentRoutePath, needChangeHistory = true) => {
        this.currentRoutePath = this.list().some(
            (val) => val.routePath === routePath,
        )
            ? routePath
            : "garage";
