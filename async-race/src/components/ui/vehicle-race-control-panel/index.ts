...
export function carIcon(carColor: string) {
  const element = new BaseComponent<HTMLElement>({
    tag: 'div',
    className: classes.carContainer,
  });
  element.node.innerHTML = `<svg width="110px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 90 62.5" enable-background="new 0 0 90 50" xml:space="preserve">
...
export default class VehicleRaceControlPanel extends BaseComponent<HTMLDivElement> {
  constructor(carName: string, carColor: string) {
    ...
    this.createContent(carName, carColor);
  }

  createContent = (carName: string, carColor: string) => {
    this.node.append(
